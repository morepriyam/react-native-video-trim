...
- `merge` uses the concat **filter** (`-filter_complex concat=n=N:v=1:a=1`) rather than the concat **demuxer** (`-f concat -c copy`). Each input is normalized to the **dominant display geometry** (most frequent WxH after rotation; ties broken by first occurrence — never just "the first clip", which would make the canvas depend on clip order) via `scale+pad+setsar+format+fps` filters before entering the concat, so clips with different dimensions, pixel formats, SARs, or frame rates merge correctly (mismatched aspect ratios get letterboxed/pillarboxed with black bars; frame rate is capped at 30 fps to prevent massive frame duplication). Callers with a fixed output contract can **pin the canvas** via `targetWidth`/`targetHeight` (+ optional `targetFps`/`targetCodec` family): the canvas is then never inferred from the clips — pin-matching clips join losslessly (the iOS passthrough fast path requires the pin to be satisfied), outliers are conformed into the pin, and if nothing matches, everything is re-encoded into a synthesized coded-upright target. A pinned merge's result carries `degraded: true` when the produced file missed the pin — e.g. Android's emergency encoder rungs (`hevc_mediacodec`, long-side-capped `mpeg4`) on devices whose H.264 hardware encoder fails; playable output is preferred over a failed merge, and the degraded-output policy belongs to the caller.
